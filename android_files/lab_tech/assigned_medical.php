...
//creating a query
$select = "SELECT m.id,m.surrogate_id,m.medical_date,m.schedule_status,s.blood_type,s.medication,c.full_name,
          c.phone_no,c.email,m.user,m.equipment_status
          FROM medical_schedule m 
          INNER JOIN surrogate_mother s ON m.surrogate_id = s.surrogate_id
          RIGHT JOIN clients c on m.surrogate_id = c.client_id  
          WHERE m.schedule_status = 'Pending' AND m.emp_id='$staffID' AND m.user = 'surrogate_mother' ORDER BY m.id DESC";
